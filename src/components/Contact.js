...
import contactImg from "../assets/img/contact-girl.png";
import 'animate.css';
...
export const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://formspree.io/f/mgvagblr", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        setStatus({ success: false, message: result?.errors?.[0]?.message || "Something went wrong." });
      }
    } catch (err) {
      setButtonText("Send");
